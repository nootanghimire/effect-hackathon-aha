<?php 
	...
	$query_check_already_liked = "SELECT * FROM likes WHERE user_id = " . p('user_id') . " AND post_id = " . p('post_id') .");" ; 

	$result_two = mysqli_query($db_link, $query_check_already_liked);

	if(mysqli_num_rows($result_two) > 0) {
		# Oops
		die(json_encode(array('error' => true, 'desc' => 'Already Liked?')));
	}

	$base_query = "INSERT INTO likes VALUES(NULL, " . p('post_id') . ", " . p('user_id') . ", " . time() . ");";
